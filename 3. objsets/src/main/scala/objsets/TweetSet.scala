...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    left.filterAcc(p, acc).union(right.filterAcc(p, newAcc))
  }
  ...
  def mostRetweeted: Tweet = {
    if(!left.isEmpty && !right.isEmpty) {
      ...
      if (leftMostTweet.retweets > rightMostTweet.retweets) {
        if (leftMostTweet.retweets > elem.retweets)
          leftMostTweet
        else
          elem
      }
      else {
        if (rightMostTweet.retweets > elem.retweets)
          rightMostTweet
        else
          elem
      }
